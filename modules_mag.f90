...
!! Shift out the magnetic perturbation variables
!Richard: scalar type magnetic perturbations
real(dl) :: delb = 0._dl
real(dl) :: pib = 0._dl
! A few variables for calculating the Alfven velocity (needed in primary modes as well)
real(dl) :: klambda = 2._dl * pi
real(dl) :: blambda = 0._dl
real(dl) :: nb_ind = -3._dl

!Vector mode anisotropic stress in units of rho_gamma
real(dl) :: vec_sig0 = 1._dl
!Vector mode shear
!Richard: moved tens0 def.
real(dl) :: tens0 = 1._dl
