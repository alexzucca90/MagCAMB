    ...
    use CAMBmain
    use NonLinear
!MagCAMB: using the module magnetic
use magnetic
#ifdef NAGF95
    use F90_UNIX
#endif
...
!SPT likelihood points!
integer nn_nB = 100, nn_BLambda = 100
integer :: ind_sample, amp_sample
real(dl) :: MagInd_max = 2.d0, MagInd_min = -2.99d0, MagAmp_max = 10.d0, MagAmp_min = 0.1d0
real(dl) :: MagIndArray(nn_nB)
real(dl) :: MagAmpArray(nn_BLambda)
real(dl) :: SPTLikelihood(nn_nB,nn_BLambda)
...
!this can be erased... for SPT we only need VECTOR MODES
...
! Initialising the SPT likelihood




!Start Sampling the likelihood


!Filling the arrays of nB and BLambda
do ind_sample=1, nn_nB
    MagIndArray(ind_sample) = MagInd_min + ind_sample * (MagInd_max - MagInd_min)/nn_nB
end do

do amp_sample=1, nn_BLambda
    MagIndArray(amp_sample) = MagAmp_min + amp_sample * (MagAmpmax - MagAmp_min)/nn_BLambda
end do
...
do ind_sample = 1, nn_nB
    ...
    magamp = MagAmpArray(amp_sample)
    magind = MagIndArray(ind_sample)

    write(*,*) "Calling MagCAMB.."
                    !compensated        ! maglrat = 4 because we don't need it...
        call mag_cls(P, 1, magamp, magind, 4.0)
    write(*,*) "..MagCAMB done."
    write(*,*) "Calling SPT likelihood"
